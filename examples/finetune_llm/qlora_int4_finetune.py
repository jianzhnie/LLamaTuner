...
from llamatuner.model import SampleGenerateCallback
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if args.do_eval or args.do_predict:
        ...
        if args.max_eval_samples is not None and len(
                eval_dataset) > args.max_eval_samples:
    ...
    if args.do_train:
        ...
        if args.max_train_samples is not None and len(
                train_dataset) > args.max_train_samples:
    ...
    return dict(train_dataset=train_dataset if args.do_train else None,
...
def train():
    ...
    trainer = Seq2SeqTrainer(
        ...
        args=training_args,
        **{k: v
           for k, v in data_module.items() if k != 'predict_dataset'},
    )
    ...
    if args.do_predict:
        ...
        with open(os.path.join(args.output_dir, 'predictions.jsonl'),
                  'w') as fout:
