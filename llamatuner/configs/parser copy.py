...
def _verify_model_args(model_args: 'ModelArguments',
    ...
    if model_args.quantization_bit is not None:
        ...
        if model_args.adapter_name_or_path is not None and len(
...
def _check_extra_dependencies(
    ...
    if model_args.mixture_of_depths is not None:
        require_version('mixture-of-depth>=1.1.6',
...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    if (training_args.do_train and finetuning_args.finetuning_type == 'lora'
            and model_args.quantization_bit is None and model_args.resize_vocab
            and finetuning_args.additional_target is None):
        logger.warning(
            'Remember to add embedding layers to `additional_target` to make the added tokens trainable.'
        )

    if training_args.do_train and model_args.quantization_bit is not None and (
            not model_args.upcast_layernorm):
        logger.warning(
    ...
    ) and finetuning_args.stage == 'dpo' and finetuning_args.ref_model is None:
        logger.warning(
