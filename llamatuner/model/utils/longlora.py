...
from transformers.models.llama.modeling_llama import (Cache, LlamaAttention,
...
def llama_attention_forward(
    ...
    **kwargs,
    ...
    if getattr(self.config, 'group_size_ratio',
               None) and self.training:  # shift
        groupsz = int(q_len * getattr(self.config, 'group_size_ratio'))
        assert (q_len % groupsz == 0
        ...
        def shift(state: torch.Tensor) -> torch.Tensor:
            state = state.transpose(
                1, 2)  # output: (bsz, seq_len, n_heads, head_dim)
            state = torch.cat(
                (
                    state[:, :, :self.num_heads // 2],
                    state[:, :, self.num_heads // 2:].roll(-groupsz // 2,
                                                           dims=1),
                ),
                dim=2,
            )
            return state.reshape(bsz * num_groups, groupsz, self.num_heads,
                                 self.head_dim).transpose(1, 2)
    ...
    attn_output = torch.matmul(
        attn_weights,
        value_states)  # (bsz, :, seq_len, :) or (bsz * n_group, :, groupsz, :)
    attn_output = attn_output.transpose(1, 2).contiguous()

    if getattr(self.config, 'group_size_ratio',
               None) and self.training:  # shift back
...
def _apply_llama_patch() -> None:
    require_version('transformers==4.40.2',
...
def configure_longlora(config: 'PretrainedConfig',
