...
def load_valuehead_params(
        path_or_repo_id: str,
        model_args: 'ModelArguments') -> Dict[str, torch.Tensor]:
    r"""
    ...
    try:
        ...
        logger.info('Failed to load {}: {}'.format(V_HEAD_SAFE_WEIGHTS_NAME,
                                                   str(err)))
    ...
    try:
        ...
        logger.info('Failed to load {}: {}'.format(V_HEAD_WEIGHTS_NAME,
                                                   str(err)))
