...
def add_z3_leaf_module(model: 'PreTrainedModel') -> None:
    ...
    require_version('deepspeed>=0.13.0',
    ...
    if getattr(model.config, 'model_type', None) == 'jetmoe':
        from transformers.models.jetmoe.modeling_jetmoe import (JetMoeMoA,
                                                                JetMoeMoE)
...
def configure_moe(config: 'PretrainedConfig', model_args: 'ModelArguments',
                  is_trainable: bool) -> None:
    if model_args.moe_aux_loss_coef is not None:
        if getattr(config, 'model_type',
                   None) in ['jamba', 'mixtral', 'qwen2_moe']:
