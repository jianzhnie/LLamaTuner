...
def _gradient_checkpointing_enable(
    ...
    if not self.supports_gradient_checkpointing:
        raise ValueError('{} does not support gradient checkpointing.'.format(
            self.__class__.__name__))

    if gradient_checkpointing_kwargs is None:
        gradient_checkpointing_kwargs = {'use_reentrant': True}

    gradient_checkpointing_func = partial(checkpoint,
    ...
    if ('value' in inspect.signature(
            self._set_gradient_checkpointing).parameters):  # old GC format
...
def _fp32_forward_post_hook(module: 'torch.nn.Module',
...
def prepare_model_for_training(
    ...
    if model_args.upcast_layernorm:
        ...
        for name, param in model.named_parameters():
            if param.ndim == 1 and any(ln_name in name
    ...
    if not model_args.disable_gradient_checkpointing:
        if not getattr(model, 'supports_gradient_checkpointing', False):
            ...
            model.gradient_checkpointing_enable(
                gradient_checkpointing_kwargs={'use_reentrant': True})
            setattr(model.config, 'use_cache',
