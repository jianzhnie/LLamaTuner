...
from llamatuner.model.utils import (
    add_z3_leaf_module, configure_attn_implementation, configure_longlora,
    configure_moe, configure_quantization, configure_rope,
    prepare_model_for_training, prepare_valuehead_model,
    print_attn_implementation, resize_embedding_layer)
...
def patch_config(
    ...
    if is_torch_npu_available():
        use_jit_compile = os.environ.get('JIT_COMPILE',
                                         '0').lower() in ['true', '1']
    ...
    if (getattr(config, 'model_type', None) == 'qwen2' and is_trainable
            and model_args.flash_attn == 'fa2'):
        setattr(
            config, 'use_cache',
            False)  # qwen2 does not support use_cache when using flash attn
    ...
    if not is_deepspeed_zero3_enabled() and not is_fsdp_enabled():
        # cast dtype and device if not use zero3 or fsdp
...
def patch_model(
    ...
    if not gen_config.do_sample and (
        (gen_config.temperature is not None and gen_config.temperature != 1.0)
            or (gen_config.top_p is not None and gen_config.top_p != 1.0) or
        (gen_config.typical_p is not None and gen_config.typical_p != 1.0)):
