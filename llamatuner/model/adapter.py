...
def init_adapter(
    ...
    if (is_deepspeed_zero3_enabled() or is_fsdp_enabled()
            or finetuning_args.pure_bf16 or finetuning_args.use_badam):
        logger.info(
    ...
    if finetuning_args.finetuning_type == 'full' and is_trainable:
        ...
        for name, param in model.named_parameters():
            if not any(forbidden_module in name
    ...
    if finetuning_args.finetuning_type == 'freeze' and is_trainable:
        ...
        if finetuning_args.use_llama_pro:
            ...
            trainable_layer_ids = range(stride - 1, num_layers + stride - 1,
                                        stride)
        elif (finetuning_args.freeze_trainable_layers >
              0):  # fine-tuning the last n layers if num_layer_trainable > 0
        ...
        for name, param in model.named_parameters():
            if any(trainable_layer in name
                   for trainable_layer in trainable_layers) and not any(
                       forbidden_module in name
                       for forbidden_module in forbidden_modules):
                if cast_trainable_params_to_fp32:
                    param.data = param.data.to(torch.float32)
            else:
        ...
        logger.info('Set trainable layers: {}'.format(','.join(
            map(str, trainable_layer_ids))))

    if finetuning_args.finetuning_type == 'lora':
        logger.info('Fine-tuning method: {}'.format(
        ...
        if model_args.adapter_name_or_path is not None:
            ...
            if getattr(model, 'quantization_method',
                       None):  # merge lora in quantized model is unstable
        ...
        if (is_trainable and adapter_to_resume is
                None):  # create new lora weights while training
            if (len(finetuning_args.lora_target) == 1
            ...
            if (finetuning_args.use_dora
                    ...
                    and getattr(model, 'quantization_method',
                                None) != QuantizationMethod.BITS_AND_BYTES):
                raise ValueError(
        ...
        if model_args.adapter_name_or_path is not None:
            logger.info('Loaded adapter(s): {}'.format(','.join(
