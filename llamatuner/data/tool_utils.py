...
GLM4_TOOL_PROMPT = ('你是一个名为 ChatGLM 的人工智能助手。你是基于智谱AI训练的语言模型 GLM-4 模型开发的，'
...
class DefaultToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: List[Dict[str, Any]]) -> str:
        ...
        for tool in tools:
            ...
            for name, param in tool['parameters']['properties'].items():
                ...
                if param.get('enum', None):
                    enum = ', should be one of [{}]'.format(', '.join(
                        param['enum']))
        ...
        return DEFAULT_TOOL_PROMPT.format(tool_text=tool_text,
...
class Llama3ToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: List[Dict[str, Any]]) -> str:
        ...
        for tool in tools:
            ...
            tool_text += json.dumps(wrapped_tool, indent=4,
                                    ensure_ascii=False) + '\n\n'
    ...
    def function_formatter(
        ...
        return [
            f'{{"name": "{functions[0].name}", "parameters": {functions[0].arguments}}}'
        ]
...
class MistralToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: List[Dict[str, Any]]) -> str:
        ...
        return '[AVAILABLE_TOOLS] ' + json.dumps(
            wrapped_tools, ensure_ascii=False) + '[/AVAILABLE_TOOLS]'
